...
impl TonicArticleService for ArticleService {
    async fn create_article(
        ...
        let mut series: Option<model::Series> = None;
        ...
        let mut categories: Vec<model::Category> = vec![];
        ...
        let mut tags: Vec<model::Tag> = vec![];

        if !tag_ids.is_empty() {
            model::Article::add_tags(&mut transaction, article.id, tag_ids.clone())
                .await
                .map_err(|why| {
            ...
            tags = sqlx::query_as("SELECT * FROM article.tags WHERE id = ANY($1)")
                .bind(tag_ids)
                .fetch_all(&mut *transaction)
                .await
                .map_err(|why| {
                    tracing::error!("Failed to get tags: {}", why);
                    Status::internal("Internal Error")
                })?;
        }
